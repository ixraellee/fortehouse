...
session_start();
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['new_admin']) and !empty($_POST['new_admin'])){
        $new_admin = $_POST['new_admin'];
        $check = $dbconnect->query("SELECT * FROM users WHERE `email`='$new_admin'");
        if ($check) {
            if(mysqli_num_rows($check) == 0){
                echo 'user doesnt exist';
            }else{
                $new = 1;
                $update = $dbconnect->query("UPDATE users SET `admin`='$new' WHERE `email`='$new_admin'");
                if ($update) {
                    $res = true;
                }
            }
            ...
            <li class=" has-sub-menu">
              ...
              <div class="sub-menu-w">
                ...
                <div class="sub-menu-i">
                  <ul class="sub-menu">
                    <li>
                      <a href="grant_security_code_request.php">Grant Security Code Request</a>
                    </li>
                    <li>
                      <a href="#">Messages</a>
                    </li>
          ...
          <div class="content-i">
            <div class="content-box">
              ...
              <div class="row">				
                ...
                <?php 
                    if ($res) { ?>
                        <div class="alert alert-success text-white">User with Email <?php echo $_POST['new_admin']; ?> has been granted Admin rights</div>
                    <?php }
                ?>
                      ...
                      <form id="transfer-form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">								
                              <label class="lighter" for="">Email</label><input class="form-control" data-error="Enter Correct Account Info" placeholder="Enter email" required type="email" name="new_admin" value="">                                
                            </div>
                        </div>

                        
						<button class="mr-2 mb-2 btn btn-primary" id="transfer-btn"  type="submit">Grant priviledge<span></span><i class="os-icon os-icon-grid-18"></i></button>
							<!--div class="form-group">
